...
const RoomsSection = () => {
    ...
    // --- State for the new filtering functionality ---
    ...
    if (isLoading) {
        return <div className="text-center py-20 text-text-muted">Loading Accommodations...</div>;
    }
    ...
    return (
        <section id="rooms" className="py-20 bg-brand-light">
            <div className="container mx-auto text-center">
                ...
                {/* --- NEW: Professional & Responsive Filter UI --- */}
                <motion.div 
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }} 
                    className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-12 p-4 bg-white rounded-lg shadow-md max-w-2xl mx-auto"
                >
                    <div className="w-full sm:w-1/2">
                        <label className="block text-sm font-medium text-left text-text-dark">Room Type</label>
                        <select name="type" value={filters.type} onChange={handleFilterChange} className="w-full p-2 border rounded-md mt-1">
                    ...
                    <div className="w-full sm:w-1/2">
                        <label className="block text-sm font-medium text-left text-text-dark">Max Price: <span className="font-bold text-brand-accent">${filters.maxPrice}</span></label>
                        <input type="range" name="maxPrice" min="100" max="1000" step="50" value={filters.maxPrice} onChange={handleFilterChange} className="w-full mt-2 h-2 bg-brand-subtle rounded-lg appearance-none cursor-pointer" />
                    </div>
                </motion.div>
